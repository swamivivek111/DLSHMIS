...
export default function DesignationForm() {
  ...
  const handleSubmit = form.onSubmit(async (values:any) => {
    try {
      /*if (isEdit) {
        await updateDesignation(Number(id), values);
  ...
  return (
    <Container>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <TextInput label="Designation Name" {...form.getInputProps('designationName')} />
        <TextInput label="Designation Code" {...form.getInputProps('designationCode')} />
