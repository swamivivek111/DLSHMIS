...
const TariffMaster: React.FC = () => {
  ...
  const handleSubmit = async (values: Tariff) => {
    ...
    try {
      ...
      if (editingTariff) {
        await axios.put(`http://localhost:9000/master/tariffs/${editingTariff.tariffId}`, values, { headers });
        notifications.show({
          title: 'Success',
        ...
        await axios.post('http://localhost:9000/master/tariffs', values, { headers });
        notifications.show({
          title: 'Success',
