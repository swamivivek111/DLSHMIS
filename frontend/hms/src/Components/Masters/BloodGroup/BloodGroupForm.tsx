...
export default function BloodGroupForm() {
  ...
  const loadData = async () => {
    try {
      ...
      const hospitals = hospitalRes.hospitals || hospitalRes.data || [];
      ...
      // Load blood group data after dropdowns are populated
      if (isEdit && id) {
        setTimeout(() => loadBloodGroupData(), 100);
      }
    } catch (error) {
  ...
  const loadBloodGroupData = async () => {
    try {
