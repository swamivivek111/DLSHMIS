...
import { countrySpecializations } from '../../../DataMaster/DropdownData';
export default function CountryForm() {
  ...
  const handleSubmit = form.onSubmit(async (values:any) => {
    try {
      /*if (isEdit) {
        await updateCountry(Number(id), values);
  ...
  return (
    <Container>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <TextInput label="Country Code" {...form.getInputProps('countryCode')} />
